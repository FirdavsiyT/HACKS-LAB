...
from .forms import ChallengeForm, CategoryForm, TimerSettingsForm, MentorMessageForm, LessonTemplateForm
...
def dashboard(request):
    ...
    if lesson_settings.start_time and lesson_settings.end_time:
        ...
        initial_data = {'duration_minutes': int(round(duration)), 'delay_minutes': int(round(delay))}
...
def send_message(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if recipient == 'all':
                ...
                if len(broadcasts) > 20: broadcasts = broadcasts[-20:]
    ...
    return render(request, 'mentors/message_form.html',
                  {'form': form, 'title': 'Send Message', 'users_list': users_list})


@login_required
def check_messages(request):
    if not request.user.is_authenticated: return JsonResponse({'messages': []})
    new_messages = []
...
def challenges_list(request):
    ...
    if category_filter: challenges = challenges.filter(category__name=category_filter)
    if sort_by == 'category':
    ...
    context = {'challenges': challenges, 'categories': categories, 'current_sort': sort_by,
...
def export_challenges_docx(request):
    if Document is None:
        messages.error(request, "Library 'python-docx' is missing.")
    ...
    if not challenges.exists():
        ...
        return redirect('mentors:challenges_list')
    document = Document()
    document.add_heading('HackLabs Active Challenges', 0).alignment = 1
    document.add_paragraph(f"Exported on: {timezone.now().strftime('%Y-%m-%d %H:%M')}").alignment = 1
    document.add_paragraph("-" * 50).alignment = 1
    for challenge in challenges:
        p = document.add_paragraph()
        p.space_after = Pt(12)
        p.add_run("Title: ").bold = True
        p.add_run(f"{challenge.title}\n")
        p.add_run("Category: ").bold = True
        p.add_run(f"{challenge.category.name if challenge.category else 'Uncategorized'}\n")
        p.add_run("Description: ").bold = True
        p.add_run(f"{challenge.description}\n")
        p.add_run("Flag: ").bold = True
        p.add_run(f"{challenge.flag}\n")
        p.add_run("Award: ").bold = True
        p.add_run(f"{challenge.points} Pts")
        document.add_paragraph("_" * 30).alignment = 1
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.wordprocessingml.document')
    response[
        'Content-Disposition'] = f'attachment; filename="challenges_export_{timezone.now().strftime("%Y%m%d")}.docx"'
...
def users_list(request):
    ...
    for user in users_qs:
        user.percentage = (user.total_points / max_possible_points * 100) if max_possible_points > 0 else 0.0
        users.append(user)
    return render(request, 'mentors/users_list.html', {'users': users, 'max_possible_points': max_possible_points})
...
def export_users_csv(request):
    ...
    response[
        'Content-Disposition'] = f'attachment; filename="hacklabs_results_{timezone.now().strftime("%Y-%m-%d_%H-%M")}.csv"'
    ...
    users = User.objects.filter(is_superuser=False).exclude(groups__name='Mentors').annotate(
        total_points=Coalesce(Sum('solves__challenge__points'), 0)).order_by('-total_points')
    for index, user in enumerate(users, 1):
        percentage = (user.total_points / max_possible_points * 100) if max_possible_points > 0 else 0.0
        writer.writerow([index, user.username, user.total_points, max_possible_points, f"{percentage:.2f}%"])
    return response
...
def reset_platform(request):
    ...
    with transaction.atomic():
        ...
        lesson_settings.hard_deadline = None
        lesson_settings.save()
        deleted_count, _ = User.objects.filter(is_superuser=False, is_staff=False).exclude(
            groups__name='Mentors').delete()
        messages.success(request, f'Platform Reset Complete! Removed {deleted_count} users.')
    return redirect('mentors:users_list')